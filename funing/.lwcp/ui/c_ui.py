...
class Cui:
    ...
    def set_ui(self):
        ...
        self.showf_optionmenu = OptionMenu(
            self.cui_frame,
        ...
        self.ct_label = Label(self.cui_frame, text=_("tolerance") + ":")
        self.ct_stringvar = StringVar(self.cui_frame, "")
        self.ct_entry = Entry(
            self.cui_frame, width=8, textvariable=self.ct_stringvar
        )

        # shoot
        self.pp_sv = StringVar(self.cui_frame, _("Pause"))
        self.pp_btn = Button(self.cui_frame, textvariable=self.pp_sv)

        self.pick_sv = StringVar(self.cui_frame, _("Pick"))
        self.pick_btn = Button(self.cui_frame, textvariable=self.pick_sv)
