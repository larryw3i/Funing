...
class ScrollableFrame(Frame):
    def __init__(self, container, orient=0, *args, **kwargs):
        ...
        scrollbar = Scrollbar(
            top_frame if orient == 0 else bottom_frame,
            command=canvas.yview if orient == 0 else canvas.xview,
            orient=VERTICAL if orient == 0 else HORIZONTAL,
        )
        if orient == 0:
        ...
        scrollbar.pack(
            side=RIGHT if orient == 0 else BOTTOM, fill=Y if orient == 0 else X
        )
