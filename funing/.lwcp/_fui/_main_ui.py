...
class _MainUI:
    ...
    def about_fn(self):
        if self.about_tl == None:
            ...
            Label(
                self.about_tl, text=_("Licensed under the MIT license")
            ).pack()
    ...
    def set_ui_events(self):
        self.rbmixfm.lang_combobox.bind(
    ...
    def pick(self):
        ...
        while True:
            ...
            if ret:
                ...
                count += 1
                if count > self.face_enter_count:
                    break
    ...
    def refresh_frame(self):
        ...
        vid_img = cv2.resize(frame, (0, 0), fx=self.fxfy, fy=self.fxfy)
    ...
    def show_nsrc_error(self):
        ...
        messagebox.showerror(
            unable_open_s, unable_open_s + ": " + self.showf_sv
        )
    ...
    def change_face_show(self, _as, rec=True):
        if len(self.face_frames) > 0:
            ...
            self.curf_index = (
                0
                if self.curf_index < 0
                else self.face_enter_count - 1
                if self.curf_index >= self.face_enter_count
                else self.curf_index
            )

            vid_img = cv2.cvtColor(
                self.face_frames[self.curf_index], cv2.COLOR_BGR2RGB
            )
            vid_img = Image.fromarray(vid_img)
            imgtk = ImageTk.PhotoImage(image=vid_img)
            ...
            roi_gray = cv2.resize(
                roi_gray, (92, 112), interpolation=cv2.INTER_LINEAR
            )
            ...
            self.infofm.face_text.delete(1.0, END)
            self.infofm.face_text.insert(
                "1.0", open(info_file_path, "r").read()
            )
            self.update_num_label()
    ...
    def add_face_label_r(self, num):
        ...
        new_fl.bind(
            "<Double-Button-1>", lambda e: self.del_face_label_r(e, index)
        )
