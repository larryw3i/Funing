...
class MainApplication(pygubu.TkApplication):
    def _create_ui(self):
        ...
        self.face_frame = builder.get_object(
            "face_frame", self.master
        )  # not vid frame, it shows picture.
    ...
    def data(self):
        ...
        if self.data_tkapp is None:
            ...
            self.data_tkapp.master.protocol("WM_DELETE_WINDOW", self.data)
    ...
    def add_face_label_pick(self, index):
        ...
        menu.add_command(
            label=_("delete"),
            command=(
                lambda label=new_face_label, index=index: self.del_face_label_pick(
                    label, index
                )
            ),
        )
    ...
    def show_from_opt_command(self, *args):
        ...
        show_from = self.get_dict_key_by_value(
            self.show_from_opt_dict, self.show_from_opt_stringvar.get()
        )
        if show_from == "file":
            self.face_src_path = tkf.askopenfilename(
                title=_("Select a file"),
    ...
    def add_face_label_rec(self, f_index):
        ...
        self.cur_info_id = cur_info_id = self.info_ids[
            _label
        ]  # global self.x and x are different.
    ...
    def pick(self):
        ...
        self.status_label_stringvar.set(
            self.var_face_was_detected_str + f"({self.var_click_to_remove_p})"
        )
